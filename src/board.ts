...
import { type GameSettings, initGameSettings } from "./externalDSLParser.ts";
...
export default class Board {
  ...
  addTile(tile: BoardTile) {
    ...
    if (tile.tileColor == 1) {
      this.ctx!.fillStyle = this.gameSettings!.board_info.board_colors[0]!;
  ...
  drawTile(ctx: CanvasRenderingContext2D, tile: BoardTile) {
    if (tile.tileColor == 1) {
      ctx.fillStyle = this.gameSettings!.board_info.board_colors[0]!;
  ...
  getSpace(pos: Position) {
    const found = this.tiles.find((tile) =>
      tile.xPos == pos.x && tile.yPos == pos.y
    );
    if (found) {
      return found;
    } else {
